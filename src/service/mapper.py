...
class AxisMapper:
    def __init__(self, max_val: float, sensitivity: float = 1.0, smoothing_window: int = 5, invert: bool = False):
        """
...
class InputMapper:
    def __init__(self, mode="landscape"):
        ...
        if self.mode == "landscape":
            ...
            self.steering = AxisMapper(max_val=6.0, invert=False, smoothing_window=1)
        else:
            # Portrait mode (Hold phone vertically)
            # In portrait, tilting right makes X negative. So we MUST invert it for gamepads.
            self.steering = AxisMapper(max_val=6.0, invert=True, smoothing_window=1)
