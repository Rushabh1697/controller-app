...
- **Dynamic Background Contrast & Reset**: Integrated `palette_generator` to calculate the luminance of the custom background image. The UI now intelligently flips all glassmorphism buttons, borders, and text to a dark/black glassy style if the image is bright, ensuring the UI is never camouflaged or cut off. Also added a "Remove Background" option in Settings to quickly revert to the default gradient.
